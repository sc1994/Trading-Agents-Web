...
> 本计划必须先通过当前实施方案的独立设计审查；审查通过和设计 PR 合并前不得执行 Task 1-5。
...
**架构：** 设计包以已核验、只含用户 README 的 GitHub `main` 独立根提交为唯一 PR base，source head 从该 base 重建且 diff 只含批准 spec 与 plan。Task 1 再用一次无关历史合并连接上游签名 tag 对应提交，通过显式组合器完整保留用户 README、上游 README 并加入 lineage；GitHub workflow 只普通推送 `HEAD:main`；Gitea workflow 从完整本地 checkout 解析受保护 `main` SHA，并调用可单元测试的发布控制器复用或拒绝既有不可变镜像、执行离线 smoke、保护 stable、保留三版和回滚。

**技术栈：** Git/GitHub CLI、GitHub Actions、Gitea Actions/`gitea-runner-ai`、Bash、Python 3.10-3.13、pytest、PyYAML、ruff、Docker Engine。

**规格：** `docs/design/Trading-Agents-Web-repository-initialization-gitea-local-release-design-spec-v1.0.0.md`，规格版本 `v1.0.0`，SHA-256 `45c70bb927b030003ca91f5abfc88aef515d7dc942c774dbe24a91f6fdcc7a2f`。
...
- 构建或 smoke 失败不得改变 `local-stable` 或清理旧成功镜像；回滚只重指稳定别名，不重新构建。
...
- 设计初期 GitHub 仓库为空；随后仓库 Admin 创建了默认 `main` 和 `README.md`。2026-09-02 fresh-fetch 后 `origin/main` 固定为独立根提交 `76fc9e407842970e8e6fdfdf32a2f9b7ef86be13`，不得再执行旧空 tree bootstrap。
- 该 base 无 parent，tree 为 `4fc3b335cf19a02aed5caffb6eb22962832189ef`，只含 `README.md`；README blob 为 `f28ff51c7392b770bbd7ac16024c7fb4d8b67dc2`，精确内容为 `# Trading-Agents-Web\n`。
- 上游 `v0.4.0` 同时存在同名 branch；只使用 `refs/tags/v0.4.0`，不得使用 `git clone --branch v0.4.0` 来选择基线。
- 上游 `v0.4.0` README blob 为 `505b69df46ce78e6bb0b22088a5b9c380cbc7a39`，与用户 README 是同路径独立添加；Task 1 必须显式解决唯一的 add/add 冲突，不得选择 `--ours`、`--theirs` 或静默覆盖。
...
## 已建立 README bootstrap 基线与设计 PR 门禁

GitHub `main` 已由仓库 Admin 建立；本计划接受且只接受以下完整基线身份：

```text
BASE_REF=refs/heads/main
BASE_SHA=76fc9e407842970e8e6fdfdf32a2f9b7ef86be13
BASE_TREE=4fc3b335cf19a02aed5caffb6eb22962832189ef
BASE_README_BLOB=f28ff51c7392b770bbd7ac16024c7fb4d8b67dc2
BASE_README_BYTES=# Trading-Agents-Web\n
```

旧计划中的空 tree root `0e3a4b07f79c3f95de41f54ab56c0bfbd782f107` 已被真实远端状态取代，不得再推送、合并、force 到 `main` 或作为 PR base。每次重建、复审或创建 PR 前都先 fresh-fetch 并执行：
...
test "$(git rev-list --parents -n 1 "$BASE_SHA")" = "$BASE_SHA"
test "$(git rev-parse "${BASE_SHA}^{tree}")" = \
  "4fc3b335cf19a02aed5caffb6eb22962832189ef"
test "$(git ls-tree -r --name-only "$BASE_SHA")" = "README.md"
test "$(git rev-parse "${BASE_SHA}:README.md")" = \
  "f28ff51c7392b770bbd7ac16024c7fb4d8b67dc2"
test "$(git show "${BASE_SHA}:README.md")" = "# Trading-Agents-Web"
```

Expected: 七项身份检查全部退出 0。任何 branch 前进、tree/file/blob/content 差异都停止；不得修改、覆盖或回退远端 `main` 来适配旧设计 head。

本次设计 source 必须从上述 base 重建，不把旧空 tree root 或旧 plan commit 带入新历史。重建记录如下；这里只操作本地隔离 checkout，不 push、不创建 PR：

```bash
set -euo pipefail
BASE_SHA="76fc9e407842970e8e6fdfdf32a2f9b7ef86be13"
OLD_PLAN_HEAD="6ae3a50e5eb6ed1f85b64376d25faa6810e55d16"
git switch -c design/codi-38-plan-on-readme-main \
  "refs/remotes/origin/main"
git restore --source="$OLD_PLAN_HEAD" -- \
  docs/design/Trading-Agents-Web-repository-initialization-gitea-local-release-design-spec-v1.0.0.md \
  docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.0.2.md
mv docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.0.2.md \
  docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.0.3.md
```

修订提交后，设计复审必须 fresh-fetch 同一 base 并同时验证 source head 的单一 parent、PR diff 和 README 未改：

```bash
set -euo pipefail
BASE_SHA="76fc9e407842970e8e6fdfdf32a2f9b7ef86be13"
PLAN_HEAD="$(git rev-parse HEAD)"
test "$(git rev-parse "${PLAN_HEAD}^")" = "$BASE_SHA"
test "$(git rev-list --count "${BASE_SHA}..${PLAN_HEAD}")" = "1"
test "$(git rev-parse "${PLAN_HEAD}:README.md")" = \
  "f28ff51c7392b770bbd7ac16024c7fb4d8b67dc2"
...
test "$(sha256sum \
  docs/design/Trading-Agents-Web-repository-initialization-gitea-local-release-design-spec-v1.0.0.md | awk '{print $1}')" = \
  "45c70bb927b030003ca91f5abfc88aef515d7dc942c774dbe24a91f6fdcc7a2f"
```

设计与 PR 门禁顺序固定为：

1. 设计复审返回已审查 `PLAN_HEAD`、目标分支 `main` 和 fresh-fetch 目标版本 `76fc9e407842970e8e6fdfdf32a2f9b7ef86be13`。
2. 只有三元组完整且当前 head/base 未漂移时，PR 阶段才普通推送该已审查 head 到独立 source branch；禁止 rebase、force-push 或把旧 `6ae3a50e...` history 推入 source。
3. 创建 PR 后 provider head/base 必须分别等于已审查 head 和 `main`，且 diff 只能包含批准 spec 与 v1.0.3 plan；`README.md` 不得出现在设计 PR diff。
4. 若 `origin/main` 不再等于固定 base，或 source head 有其他 parent/file，停止并重新设计审查；不得创建或替换 PR。
5. 设计 PR 合并后，Task 1 才从新的 `origin/main` 建实现分支，显式组合用户 README、lineage 和上游 README。

## 文件职责映射

- `UPSTREAM.md`：版本化保存上游 URL、tag object、commit、签名核验、许可证和人工同步命令。
- `README.md`：Task 1 显式组合并完整保留用户 bootstrap README、`Repository lineage` 和上游 `v0.4.0` README，不静默选择任一冲突侧。
...
- `scripts/local_image_release.py`：唯一 Docker 镜像生命周期入口，封装不可变 SHA 复用/冲突拒绝、`publish`、`rollback`、stable 感知保留和使用中镜像保护。
- `docs/operations/repository-sync-and-local-release.md`：用户配置、触发、审计、故障处理、回滚和安全清理 runbook。
...
- `tests/infra/test_local_image_release.py`：用 fake Docker runner 验证失败保护、不可变 SHA 重跑/冲突拒绝、stable 感知保留、使用中镜像和回滚。
...
- `tests/infra/test_operations_runbook.py`：验证 runbook 包含全部用户配置与验收命令，且不含任何 Secret 值。
...
- `publish` 首先检查完整 SHA tag：不存在时才构建 candidate；存在且 managed/revision 标签匹配时直接复用并重新离线 smoke；存在但标签不匹配时失败，绝不覆盖。
- 新 candidate 通过 smoke 后再次检查完整 SHA tag，防止检查与打 tag 之间的竞态；只在 tag 仍不存在时创建，不把不同 image ID 覆盖到既有 SHA。
- smoke 对 candidate 或复用的不可变 SHA 执行 `--network none`、只读根文件系统、无宿主机 env/volume 的 `tradingagents --help`。
- smoke 成功后更新 `local-stable`；此前不触碰稳定别名。完整 SHA tag 永远不被重写。
...
- `rollback` 只接受存在且 managed/revision 标签匹配的完整 SHA 镜像，然后执行一次 Docker tag 更新稳定别名。

## Stage 与依赖

| Stage | 实现任务 | 依赖 | 独立审查产物 |
|---|---|---|---|
| 1 | Task 1：导入精确上游历史与来源证明 | 设计审查记录 `origin/main@76fc9e407842970e8e6fdfdf32a2f9b7ef86be13`、仅 spec/plan 的设计 PR 已合并 | README 无损组合、上游基线 merge commit、来源文档、许可证与独立根保护测试 |
| 2 | Task 2：GitHub→Gitea 同步；Task 3：本地镜像控制器 | Task 1 | 两项可并行审查，分别通过 bare-repo 测试和不可变 SHA/stable 感知 fake-Docker 测试 |
| 3 | Task 4：Gitea 门禁与发布编排 | Task 2、Task 3 | 平台隔离、私有 checkout 无持久凭据、恶意输入拒绝、四版本门禁、串行发布和人工回滚 workflow |
| 4 | Task 5：运维 runbook 与端到端验收证据 | Task 4，用户已配置 token、分支保护和 Runner | 脱敏的 GitHub/Gitea job URL、SHA/镜像检查与失败保护记录 |

## 迁移与兼容性

- 这是空仓初始化，没有数据库、API、配置格式或 TradingAgents 运行数据迁移。
- 已建立的 README bootstrap root `76fc9e407842970e8e6fdfdf32a2f9b7ef86be13` 是设计 PR 的固定 base；旧空 tree root 不再进入任何远端历史。设计 PR 合并后，Task 1 仍须证明该 root 可达且 `README.md` blob 未被设计 PR 改写。
- 设计文档历史与上游历史通过 `--allow-unrelated-histories` 的非压缩 merge commit 连接；README add/add 冲突必须由版本化组合器解决，最终文件以用户 README 原始字节开头、以上游 README 原始字节结尾，中间加入 lineage；不得用 `--ours`、`--theirs` 或整文件覆盖。不得 squash 掉上游 273 个可达提交。
- 导入后 Python/package/CLI/Docker 行为保持上游 `v0.4.0`；基础设施文件只增加 CI、发布和文档，不改业务模块。
- Gitea 目标必须继续为空或能从 GitHub `main` 快进。发现 Gitea 已有独立提交时停止，不重置、不 force-push。
- 后续人工同步上游时先 fetch，再在功能分支合并或 cherry-pick，经 GitHub PR 审查进入 `main`；Gitea 仍只接收 GitHub `main` 的普通推送。

---

### Task 1：导入 TradingAgents v0.4.0 精确历史并固化来源
...
- [ ] **Step 1：确认设计 PR 合并结果仍完整保留已核验 README root**
...
- [ ] **Step 2：精确 fetch 同名 tag，而不是同名 branch，并验证 GitHub 签名状态**
...
- [ ] **Step 4：实现只在 add/add 冲突期间工作的 README 无损组合器**
...
- [ ] **Step 5：开始无关历史 merge，显式解决 README 并添加来源记录**
...
- [ ] **Step 6：运行来源测试，确认 merge tree 满足约束**
...
### Task 2：实现 GitHub main 到 Gitea main 的失败关闭同步

**Stage：** 2（可与 Task 3 并行）
...
- [ ] **Step 1：添加结构化 workflow 测试依赖和失败测试**
...
- [ ] **Step 3：添加 GitHub 平台 guard、最小权限和 Secret 边界**
...
### Task 3：实现失败保护的本地镜像发布与回滚控制器

**Stage：** 2（可与 Task 2 并行）
...
- Produces: Task 4 调用的 `publish`/`rollback` CLI；不读取 workflow Secret 或 TradingAgents 应用配置。

- [ ] **Step 1：写失败保护、不可变重跑、stable 感知保留和回滚测试**
...
Expected: 10 tests PASS；其中既有 SHA 复用、冲突/竞态拒绝、旧 SHA 回滚后清理和 stable traceability 回归均通过；ruff 退出 0。
...
git commit -m "feat: manage local release image lifecycle"
```

Expected: commit 不包含 workflow、Secret 或宿主机状态文件。
...
- Produces: `resolve_release_target.py` 只基于完整本地 checkout 输出已校验的 `target_sha`/`operation`；`push(main)` 自动 publish；`workflow_dispatch(operation=publish|rollback,target_sha=<40hex>)` 人工重跑/回滚。
...
Expected: resolver 5 tests、workflow 5 tests和控制器 10 tests 全部 PASS；私有仓库无持久凭据、本地祖先校验、恶意输入、不可变重跑与 stable 保护回归均通过；ruff 与 diff check 退出 0。
...
### Task 5：交付运维 runbook 并收集端到端验收证据
...
- Produces: 维护者可重复执行的初始化后同步、人工重跑、审计、回滚和安全清理流程，以及与验收标准一一对应的证据位置。

- [ ] **Step 1：写 runbook 完整性失败测试**
...
def test_runbook_covers_configuration_and_recovery_without_secret_values() -> None:
...
Expected: FAIL，因为 runbook 尚不存在。

- [ ] **Step 2：编写用户配置、正常发布和审计流程**
...
GitHub `sc1994/Trading-Agents-Web` `main` is authoritative. Gitea `suncheng/Trading-Agents-Web` is a private, one-way target. Never resolve divergence with force, mirror, ref deletion, or a reverse push. The release does not start an application container, read runtime `.env` values, or access `~/.tradingagents`.
...
Open the Gitea Actions `Qualify and publish local image` workflow on `main`, choose `workflow_dispatch`, set operation to `publish`, and enter a full 40-character commit SHA already contained in Gitea `main` history. The resolve job rejects short, unknown, or non-ancestor values.
...
List retained full SHA tags and choose one whose managed/revision labels match:
...
The same operation is available through Gitea `workflow_dispatch` with operation `rollback` and a retained full main-history SHA. Rollback retags an existing image; it does not build, run an analysis, read application keys, or modify TradingAgents data.

## Retention and safe cleanup

After a successful promotion, the controller keeps three managed full-SHA images in total. The SHA currently referenced by `local-stable` consumes one retention slot even after a rollback; the remaining slots are filled by newest `Created` values. Before removing any other tag it compares immutable image IDs against all running and stopped containers from `docker container inspect`; an in-use image is skipped and logged. An existing full SHA tag is never overwritten: matching managed metadata is reused and re-smoked, while conflicting metadata or a racing different image ID aborts publication. `local-stable`, candidate tags, unmanaged images, and any image used by a container are never selected as expired successful SHA images.

## Acceptance evidence

Retain the GitHub sync run URL, Gitea release run URL, common main SHA, tag verification result, four pytest results, clean-install/import result, ruff result, candidate/stable revision label, retained full-SHA list, failed-promotion stable ID comparison, rollback before/after revision, and confirmation that no release container or `~/.tradingagents` change was produced. Evidence contains job URLs, SHAs, stage names and sanitized errors only; never attach tokens, credential URLs, complete environment dumps or application secrets.
````
...
Expected: pytest 0 failures；ruff、Bash parse、CLI help、diff check 均退出 0；status 只包含 Task 5 文档和测试。
...
git commit -m "docs: add sync and local release runbook"
```

Expected: docs/test commit 可独立审查，不包含凭据值、截图中的敏感信息或运行时数据。

- [ ] **Step 5：在用户完成 Secret 与 Runner 配置后收集端到端证据**

Run and record sanitized results:
...
Expected: GitHub sync success；Gitea `main` 与 `GITHUB_SHA` 相同；Gitea 门禁全部通过；stable revision 与同一 SHA 相同；保留集合总数为三个并包含 stable SHA，若还有集合外旧 SHA 则日志和容器检查证明其因正在被容器使用而跳过；另有 `local-stable` alias；无发布 workflow 启动的容器。

- [ ] **Step 6：在 Runner 主机执行受控 smoke 失败和可恢复回滚验收**

先用当前 stable 作为不联网的失败镜像基础，只覆盖 ENTRYPOINT，使 smoke 确定失败：

```bash
set -euo pipefail
GITHUB_SHA="$(git rev-parse HEAD)"
STABLE_BEFORE="$(docker image inspect trading-agents-web:local-stable --format '{{.Id}}')"
FAILURE_CONTEXT="$(mktemp -d ./release-failure-context.XXXXXX)"
cleanup_failure_context() {
  rm -rf "$FAILURE_CONTEXT"
}
trap cleanup_failure_context EXIT
...
STABLE_AFTER="$(docker image inspect trading-agents-web:local-stable --format '{{.Id}}')"
test "$STABLE_BEFORE" = "$STABLE_AFTER"
test -z "$(docker image ls --quiet "trading-agents-web:candidate-${GITHUB_SHA}-*")"
```

Expected: publish 返回非零；stable image ID 前后相同；本次 candidate tag 已删除；旧成功 SHA tag 没有被清理。

当至少存在两个保留成功 SHA 时，选择一个非当前 SHA 完成回滚再恢复：
...
python scripts/local_image_release.py rollback \
  --sha "$ROLLBACK_SHA" --image trading-agents-web
...
python scripts/local_image_release.py rollback \
  --sha "$CURRENT_SHA" --image trading-agents-web
test "$(docker image inspect trading-agents-web:local-stable \
  --format '{{index .Config.Labels "org.opencontainers.image.revision"}}')" = "$CURRENT_SHA"
```

Expected: stable 先指向保留的旧 SHA，再无需重建地恢复当前 SHA；过程中没有启动分析容器、读取应用密钥或修改 `~/.tradingagents`。若初始化时只有一个成功 SHA，保留上述命令并在第二个成功发布后收集 live 回滚证据；自动测试已经先行验证回滚接口和标签校验。
...
|---|---|---|---|
| 1-2 来源、README、许可证、人工 upstream | Task 1 | README 双侧字节/独立根 fixture、merge ancestry、LICENSE hash | GitHub merge commit parents、`UPSTREAM.md` |
| 3-7 单向同步、最小权限、失败关闭 | Task 2 | YAML 结构测试、bare repo 快进/分叉测试、缺失 token 测试 | GitHub workflow 权限页、Secret 名、两端 main SHA |
| 8 平台隔离 | Tasks 2、4 | 两个 workflow 的 `github.server_url`/repository/ref guard 测试 | GitHub/Gitea 错误侧 job 为 skipped |
...
| 11-12 build、离线 smoke、稳定提升 | Tasks 3、4 | publish 顺序、不可变 SHA 复用/冲突拒绝和 `--network none` 测试 | 候选/stable revision label 与 job log |
| 13 失败不改变稳定版本 | Task 3 | smoke failure、竞态冲突无 tag 测试 | 受控失败前后 stable image ID |
| 14 最近三版和使用中保护 | Task 3 | rolled-back stable 占保留槽、缺失完整 SHA 拒绝、used-image 测试 | Runner 主机镜像/容器清单 |
| 15 人工回滚 | Tasks 3-5 | managed revision 回滚测试 | workflow_dispatch 与回滚前后 revision |
| 16 串行与过期 pending | Task 4 | 固定 concurrency group、`cancel-in-progress: false` | Gitea 并发运行记录；运行任务不被异步终止 |
| 17 不启动/不读密钥/不碰数据 | Tasks 3-5 | workflow forbidden-string 测试、无 volume/env 的 smoke 命令 | 无新增容器、无 `~/.tradingagents` 变更 |
| 18 脱敏审计 | Tasks 2、5 | askpass、runbook Secret 值检查 | GitHub/Gitea job URL 与脱敏日志抽查 |
...
```bash
sha256sum docs/design/Trading-Agents-Web-repository-initialization-gitea-local-release-design-spec-v1.0.0.md
if rg -n 'TB''D|TO''DO|implement ''later|fill in ''details|Similar to ''Task' \
  docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.0.3.md; then
  exit 1
fi
rg -n '^### Task|^\*\*Stage|^\*\*Files|^\*\*Interfaces|^- \[ \] \*\*Step' \
  docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.0.3.md
git diff --check
git status --short
```

Expected: spec SHA-256 精确为 `45c70bb927b030003ca91f5abfc88aef515d7dc942c774dbe24a91f6fdcc7a2f`；source head 的 parent 为固定 README base、设计 diff 只含 spec/plan 且 README blob 未改；Task 1 的独立根 fixture 覆盖唯一 add/add 冲突、两侧字节无损组合和双 parent merge；占位符扫描无命中；五个 Task 都有 Stage、Files、Interfaces、失败测试、预期失败、最小实现、通过验证和提交步骤；diff check 退出 0。
