...
**架构：** GitHub `main` 已完成上游历史导入和到 Gitea 的单向同步实现，本轮设计包从该真实 head 建立且只增加批准的 v1.1.0 spec 与 plan。剩余实现由可单元测试的发布控制器、受保护且仓库级串行的 Gitea workflow 和运维 runbook 构成：控制器在顺序执行及可观察冲突下复用或拒绝完整 SHA 标签、离线 smoke 后才提升 stable，并明确接受最终检查到 tag 写入之间的同 SHA 跨进程窄竞态；workflow 串行边界不扩展为宿主机全局锁。
...
本轮 source 从该 base 新建，只复制平台下载并核验的 v1.1.0 spec，再从已合并 v1.0.3 plan 生成本文件；不改生产文件，不恢复 CODI-61 head，不创建 PR：

```bash
set -euo pipefail
BASE_SHA="15969d15e0f1491d9f3c9c26c4635c004f380ed2"
git switch -c design/codi-38-plan-v1.1.0 refs/remotes/origin/main
cp ../approved-spec-v1.1.0/Trading-Agents-Web-repository-initialization-gitea-local-release-design-spec-v1.1.0.md \
  docs/design/Trading-Agents-Web-repository-initialization-gitea-local-release-design-spec-v1.1.0.md
cp docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.0.3.md \
  docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.1.0.md
```

修订提交后，独立设计复审必须验证单一 parent、完整设计 diff 和批准规格摘要：
...
test "$(git diff --name-only "$BASE_SHA" "$PLAN_HEAD" | sort)" = \
  "$(printf '%s\n' \
    docs/design/Trading-Agents-Web-repository-initialization-gitea-local-release-design-spec-v1.1.0.md \
    docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.1.0.md | sort)"
test "$(sha256sum \
...
3. 新设计 PR 标题同时包含 `CODI-36`、`CODI-38`，正文只写 `Closes CODI-38`；provider head/base 必须匹配审查快照，diff 只能包含 v1.1.0 spec 与 plan。旧 PR #1 的 merged 状态不能作为本轮成功锚点。
...
- `docs/operations/repository-sync-and-local-release.md`：用户配置、触发、审计、同 SHA 并发残余风险、串行操作约束、核对恢复、回滚和安全清理 runbook。
...
| Stage 1 | Task 3：恢复 CODI-61 的本地镜像控制器限定复审 | 新 v1.1.0 设计 PR 已合并；从 CODI-61 原来源恢复，不建重复 Issue | 顺序复用、可观察冲突拒绝、失败保护、stable 感知保留和回滚 fake-Docker 测试 |
| Stage 2 | Task 4：Gitea 门禁与发布编排（CODI-59） | Task 3 代码 PR 已合并；此前保持 parked | 平台隔离、私有 checkout、恶意输入拒绝、四版本门禁、workflow 级串行与人工回滚 |
| Stage 3 | Task 5：runbook 与端到端验收（CODI-58） | Task 4 代码 PR 已合并，用户已配置 token、分支保护和 Runner；此前保持 parked | 脱敏 job/SHA/镜像证据、残余风险操作约束、失败保护和回滚记录 |
...
- v1.1.0 仅改变同一完整 SHA 跨进程并发在最终检查至实际 tag 写入窄窗口的验收保证；CLI 参数、镜像名、标签名、保留数、workflow 输入、同步接口和凭据边界不变，不需要数据或配置迁移。
...
- Produces: 维护者可重复执行的初始化后同步、人工重跑、审计、回滚和安全清理流程；明确同 SHA 并发残余风险、workflow 串行边界、避免直接并发发布及核对/串行重跑恢复方式；证据位置与验收标准一一对应。
...
def test_runbook_covers_configuration_and_recovery_without_secret_values() -> None:
    ...
    required = (
        ...
        "Avoid direct concurrent publish",
...
The fixed Gitea concurrency group serializes runs managed by this repository workflow; it is not a host-wide lock for direct CLI invocations, other workflows, or external processes. Avoid direct concurrent publish operations for the same full SHA. The accepted residual window exists only when two publishing processes both target that SHA and a competing tag write lands after another process's final absence check but before its `docker image tag` write. In that window the later writer may redirect the full-SHA tag to a different image ID; no cross-process lock or atomic claim is provided.
...
4. Use the Gitea `workflow_dispatch` `publish` operation for the same authorized main-history SHA as a serialized publish rerun. Let it re-smoke the observed managed SHA tag and only then promote/clean. If metadata does not match, keep the workflow failed and escalate for reviewed image recovery; do not overwrite manually or widen Git/token permissions.
...
Retain the GitHub sync run URL, Gitea release run URL, common main SHA, tag verification result, four pytest results, clean-install/import result, ruff result, candidate/stable revision label, retained full-SHA list, failed-promotion stable ID comparison, rollback before/after revision, residual-risk inspection/serialized-recovery record, and confirmation that no release container or `~/.tradingagents` change was produced. Evidence contains job URLs, SHAs, stage names and sanitized errors only; never attach tokens, credential URLs, complete environment dumps or application secrets. Do not create acceptance evidence by racing two publishers in the shared namespace.
````
...
Expected: GitHub sync success；Gitea `main` 与 `GITHUB_SHA` 相同；Gitea 门禁全部通过；stable revision 与同一 SHA 相同；保留集合总数为三个并包含 stable SHA，若还有集合外旧 SHA 则日志和容器检查证明其因正在被容器使用而跳过；另有 `local-stable` alias；无发布 workflow 启动的容器。记录 workflow 固定 group 的串行证据及“非宿主机全局锁”边界，但不得在共享命名空间并发启动同 SHA publisher。

- [ ] **Step 6：在 Runner 主机执行受控 smoke 失败和可恢复回滚验收（不制造同 SHA 并发竞态）**
...
| 13 失败不改变稳定版本 | Task 3 | build/smoke failure 与最终检查可观察冲突均无 stable/成功清理测试 | 受控失败前后 stable image ID；不制造豁免竞态 |
...
| 16 workflow 串行与过期 pending | Task 4 | 固定 concurrency group、`cancel-in-progress: false` | Gitea workflow 运行记录；不声称约束宿主机外部进程 |
| 17 不启动/不读密钥/不碰数据 | Tasks 3-5 | workflow forbidden-string 测试、无 volume/env 的 smoke 命令 | 无新增容器、无 `~/.tradingagents` 变更 |
| 18 脱敏审计 | 已完成基线 B、Task 5 | askpass、runbook Secret 值检查 | GitHub/Gitea job URL 与脱敏日志抽查 |
| 19 有限同 SHA 竞态豁免 | Tasks 3-5 | 顺序复用、预存冲突、最终检查可观察冲突；不注入检查后竞态 | runbook 风险条件、非全局锁边界、停止额外发布/核对/串行重跑证据 |
...
PLAN="docs/plans/repository-initialization-gitea-local-release-implementation-plan-v1.1.0.md"
...
rg -n 'v1\.1\.0|Same-SHA concurrency residual risk|not a host-wide lock|serialized publish rerun|CODI-61|PR #3' "$PLAN"
git diff --check "$BASE_SHA" HEAD
git status --short
```

Expected: spec SHA-256 精确为 `971e8fb990cb911f11160107d887a531b2eed6d955ea156a70ad4cbaf3197754`；source head 是固定当前 `main` 的单 commit 后继且 diff 只含 v1.1.0 spec/plan；两个已完成基线各有历史执行证据但不会重跑；Task 3-5 各有依赖、Files、Interfaces、测试/实现/验证/提交或证据步骤；竞态豁免、非全局锁边界和串行恢复可检索；占位符扫描无命中；diff check 退出 0，worktree clean。
