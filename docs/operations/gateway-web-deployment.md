...
- 写请求（保存设置、测试连接、任务创建/恢复/重跑/删除）的访问控制由入口代理层负责：
  Nginx Proxy Manager 条目的 Basic Auth、网络白名单、TLS 终止等。应用本身**不绑定任何
  域名或端口**：实测条目包括 `https://trading.suncheng.online/` 与隧道条目
  `https://taw.suncheng.online:81/`，后者的代理会去掉 Host 端口并可能以明文 HTTP 上送，
  应用若按固定源比对会把这类入口误判为跨站并全部拒绝（403），表现为页面一切正常但
  保存/测试/创建任务静默失败。
- 因此应用中间件只拒绝 `Sec-Fetch-Site: cross-site` 的写请求（浏览器设置、页面脚本
  无法伪造），不再比较 `Origin` 与任何期望源。新增入口时无需改动应用代码，但必须在
  代理条目上配置凭据或网络白名单，并只在该入口验收写操作。
