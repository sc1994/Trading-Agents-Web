...
**当前没有已核验的生产私有入口证据，不得发布此工作台。** GitHub 到 Gitea 的
main 同步只同步代码；Gitea 部署工作流已移除 push 触发，只允许对 main 手动触发。
获授权的维护者必须先核验所有访问路径（包括既有公开域名、反向代理、隧道及转发端口）
均受私有入口保护，记录检查时间、入口清单、允许/拒绝访问的证据和维护者身份。
回环监听、候选健康检查和本地测试都不构成该证明。本次未连接或修改生产环境。

手动工作流的 `release_sha` 填写已批准的完整 main SHA；只有完成上述核验后，才可在
`private_ingress_verified_sha` 重复填写该 SHA，作为本次发布的明确人工核验声明。
两项分别传入 `MANUAL_RELEASE_SHA` 和 `PRIVATE_INGRESS_VERIFIED_SHA`，部署脚本在
任何外部命令之前要求两者与目标 SHA 完全一致，缺失或旧 SHA 均拒绝。直接执行脚本
同样受这两个门禁约束；不得预设为常驻环境变量或默认值。这是人工声明门禁，不会自行
发现或配置网络边界。需要单独的发布授权与核验记录才能填写，不得因合并代码而填写。
工作流仍校验所检出 commit、完整 SHA 和镜像 revision，保留原有候选验证及回退保护。
...
当前存储初始化使用兼容的建表逻辑，不自动转换旧 CLI 数据。代码同步后须通过上文
人工发布与私有入口核验门禁发布完整 SHA；候选使用独立临时卷，绝不挂载生产卷，
删除候选时同时删除临时卷。
